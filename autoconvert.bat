...
echo error level !ERRORLEVEL! for hybrid > "D:\Ryan\Documents\autoconvertlog.txt"
echo error level !ERRORLEVEL! for hybrid
IF !ERRORLEVEL! NEQ 0 (
	tasklist /fi "ImageName eq ffmpeg.exe" /fo csv 2>NUL | find /I "ffmpeg.exe">NUL
	echo error level !ERRORLEVEL! for ffmpeg >> "D:\Ryan\Documents\autoconvertlog.txt"
	...
	IF !ERRORLEVEL! NEQ 0 (
		...
		echo cleanup empty folders >> "D:\Ryan\Documents\autoconvertlog.txt"
		...
		setlocal EnableDelayedExpansion
		echo starting Hybrid with !Name! >> "D:\Ryan\Documents\autoconvertlog.txt"
		echo starting Hybrid with !Name!
		cd /d "C:\Program Files\Hybrid\"
		if not "!Name!"=="" (
			start Hybrid -global anime -autoAdd addAndStart !Name!
		)
		echo started Hybrid >> "D:\Ryan\Documents\autoconvertlog.txt"
		echo started Hybrid 
		endlocal
		exit
	)
